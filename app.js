...
// ==========================================
// 4. MATERI MANAGEMENT (MULTIPLE IMAGES + MODAL + EDIT)
// ==========================================
...
window.saveMateri = async () => {
  ...
  const images = imgInput ? imgInput.split(',').map(url => url.trim()) : [];

  try {
    if (editMateriId) {
      await updateDoc(doc(db, "materi", editMateriId), {
        title,
        images: images.length > 0 ? images : ["https://img.freepik.com/free-vector/cute-animals-holding-numbers-banner_1308-43306.jpg"],
        desc,
        updatedAt: new Date()
      });
...
window.editMateri = (id, title, imgListStr, desc) => {
  ...
  document.getElementById('materi-img').value = imgListStr;
...
// ==========================================
// 5. LATIHAN / KUIS MANAGEMENT & GOOGLE SHEETS
// ==========================================
...
window.deleteData = async (colName, id) => {
  if (confirm("Yakin ingin menghapus data ini?")) {
    try {
      await deleteDoc(doc(doc(db, colName, id)));
